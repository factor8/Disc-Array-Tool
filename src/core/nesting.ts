...
function generateTangentCandidates(
  ...
  // 3. Axis-aligned adjacent positions (same row/column as each placed disc)
  // These are tangent to exactly one disc and are NOT generated by the wall+disc cases
  // when there is slack between the disc and the far wall.
  for (const disc of placed) {
    const dr = radius + disc.diameter / 2 + spacing;
    candidates.push({ x: disc.x + dr, y: disc.y });
    candidates.push({ x: disc.x - dr, y: disc.y });
    candidates.push({ x: disc.x, y: disc.y + dr });
    candidates.push({ x: disc.x, y: disc.y - dr });
  }

  // 4. Tangent to two discs — intersection of two circles
...
function findPlacement(
  ...
  // Prefer positions aligned with an existing disc's row or column (same x or y within eps).
  // This keeps grid-like packing — wall-hugging diagonal tangent positions would otherwise
  // misalign rows and lose a slot when the sheet has slack beyond a whole disc width.
  const isRowColAligned = (cx: number, cy: number): boolean => {
    const eps = 0.05;
    for (const disc of placed) {
      if (Math.abs(cx - disc.x) < eps || Math.abs(cy - disc.y) < eps) return true;
    }
    return false;
  };

  // Three tiers: aligned-with-disc-row/col > other-disc-adjacent > wall-only
  let bestAlignedPos: { x: number; y: number } | null = null;
  ...
  for (const cand of candidates) {
    ...
    if (placed.length > 0 && isTouchingDisc(cand.x, cand.y)) {
      if (isRowColAligned(cand.x, cand.y)) {
        if (score < bestAlignedScore) { bestAlignedScore = score; bestAlignedPos = cand; }
      } else {
        if (score < bestDiscAdjacentScore) { bestDiscAdjacentScore = score; bestDiscAdjacentPos = cand; }
      }
    } else {
      if (score < bestWallOnlyScore) { bestWallOnlyScore = score; bestWallOnlyPos = cand; }
    }
  }

  const bestPos = bestAlignedPos ?? bestDiscAdjacentPos ?? bestWallOnlyPos;
